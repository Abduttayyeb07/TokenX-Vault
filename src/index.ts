...
async function rpcProvider(chain: ChainConfig) {
  ...
  for (const url of chain.rpc) {
    try {
      ...
      if (wallets[0]) await timeout(provider.getBalance(wallets[0].address), cfg.rpcTimeout, `RPC balance check timed out: ${url}`);
...
async function backfill(runtime: Runtime, token: TokenConfig) {
  ...
  const cursor = Math.max(0, (state.cursors[k]?.lastBlock ?? (cfg.startLatest ? latest : Math.max(0, latest - cfg.maxBacklog))) - cfg.backfillOverlap);
...
const filters = wallets.flatMap(wallet => [topicsFor(wallet.address, 1), topicsFor(wallet.address, 2)]).map(topics => ({ address: token.address, topics })); console.log(`Subscribing watched-wallet ${runtime.chain.display} ${token.name} Transfer events on ${token.address} (${filters.length} filters)`);
    ...
    stat.lastWsHeartbeatAt = Date.now(); stat.lastWsError = undefined;
...
function karachiDateTime() { const parts = new Intl.DateTimeFormat("en-GB", { timeZone: cfg.balanceTimezone, year: "numeric", month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit", hour12: false }).formatToParts(new Date()); return Object.fromEntries(parts.map(part => [part.type, part.value])) as Record<string, string>; }

async function balanceReport() { const now = karachiDateTime(); const reports: string[] = [`📊 Balance Report — ${now.day}-${now.month}`, `Timezone: ${cfg.balanceTimezone}`]; for (const chain of chains) { const runtime = chainRuntime.get(chain.name); reports.push(`\n━━━━━━━━━━━━━━━━━━━━\n${chain.name === "ethereum" ? "Ethereum" : "BNB Smart Chain"}`); if (!runtime) { reports.push("Unavailable"); continue; } for (const wallet of wallets) { reports.push(`\n🏦 ${wallet.label}`); try { const native = await runtime.provider.getBalance(wallet.address); reports.push(`${chain.native.padEnd(6)} ${ethers.formatEther(native)}`); } catch (error) { console.error(`Native balance report query failed for ${chain.name} ${wallet.label}: ${error instanceof Error ? error.message : String(error)}`); reports.push(`${chain.native.padEnd(6)} unavailable`); } for (const token of chain.tokens) { try { const value = await new ethers.Contract(token.address, ERC20_ABI, runtime.provider).balanceOf(wallet.address); reports.push(`${token.name.padEnd(6)} ${ethers.formatUnits(value, token.decimals)}`); } catch (error) { console.error(`Token balance report query failed for ${chain.name} ${token.name} ${wallet.label}: ${error instanceof Error ? error.message : String(error)}`); reports.push(`${token.name.padEnd(6)} unavailable`); } } reports.push(wallet.address); } } return reports.join("\n"); }

let lastBalanceReportKey = "";
function scheduleBalanceReports() { if (!cfg.balanceIds.length || !cfg.balanceTimes.length) return; setInterval(() => { const now = karachiDateTime(); const hhmm = `${now.hour}:${now.minute}`; const reportKey = `${now.year}-${now.month}-${now.day}-${hhmm}`; if (cfg.balanceTimes.includes(hhmm) && reportKey !== lastBalanceReportKey) { lastBalanceReportKey = reportKey; void balanceReport().then(report => telegram(cfg.balanceIds, report)).catch(error => console.error(`Scheduled balance report failed: ${error instanceof Error ? error.message : String(error)}`)); } }, 30000); }
...
process.on("uncaughtException", error => {
  ...
  console.error("Monitor kept alive; inspect this error and the affected RPC/WebSocket connection.");
});

main().catch(error => { console.error(`Fatal startup error: ${error instanceof Error ? error.stack ?? error.message : String(error)}`); process.exitCode = 1; });
